...
def search(client, user_query, index="bbuy_products", sort="_score", sortDir="desc", use_vector_query=None):
...
if __name__ == "__main__":
    ...
    general.add_argument('-v','--use_vector_query',
                         help='Use vector query for the search!',action=argparse.BooleanOptionalAction)
    ...
    host = args.host
    port = args.port
    use_vector_query = args.use_vector_query
    if args.user:
        password = getpass()
    ...
    for line in fileinput.input(('-', )):
        ...
        if query == "Exit":
            break
        search(client=opensearch, user_query=query, index=index_name, use_vector_query=use_vector_query)
